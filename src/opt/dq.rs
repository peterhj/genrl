...
use replay_new::{ReplayEntry, LinearReplayCache};
...
pub struct AdamDiffQWorker<E, F, V, EvalV, ValueFn>
where E: 'static + Env + EnvObsRepr<F>,
      F: 'static + SampleExtractInput<[f32]> + Deref<Target=[u8]> + Clone,
...
impl<E, F, V, EvalV, ValueFn> AdamDiffQWorker<E, F, V, EvalV, ValueFn>
  ...
  pub fn deterministic_test(&mut self, rng: &mut Xorshiftplus128Rng) {
    ...
    for _ in 0 .. 10_000 {
      ...
      let act_idx = {
        ...
        let obs = self.belief_state.clone();
